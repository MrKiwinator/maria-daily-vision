...
5. Перезапустите API. При старте в логе появится строка `Telegram: уведомления о новых новостях и планах включены`.
...
**Планы:** уведомление при **добавлении** нового плана (`POST /api/plans`) — название, описание, статус, ссылка на место (если есть), ссылка на раздел «Планы», фото (на продакшене с HTTPS). Отключить: `NOTIFY_ON_PLANS=false`.
