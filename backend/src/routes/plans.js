...
function buildListQuery(search, date) {
  let sql = 'SELECT id, title, content, image_path, published_at FROM plans WHERE 1=1';
  const params = [];
  if (search) {
    sql += ' AND title LIKE ?';
    params.push(`%${search}%`);
...
router.post('/', authenticate, requireAdmin, upload.single('image'), (req, res) => {
  ...
  const result = db
    .prepare(
      'INSERT INTO plans (title, content, image_path, published_at) VALUES (?, ?, ?, ?)'
    )
    .run(title.trim(), content.trim(), image_path, pubDate);
  const item = db.prepare('SELECT * FROM plans WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', authenticate, requireAdmin, upload.single('image'), (req, res) => {
  ...
  if (req.file) {
    const oldPath = path.join(process.cwd(), 'uploads', path.basename(item.image_path));
    if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
    image_path = `/uploads/${req.file.filename}`;
  }

  db.prepare(
    'UPDATE plans SET title = ?, content = ?, image_path = ?, published_at = ? WHERE id = ?'
  ).run(newTitle, newContent, image_path, published_at || item.published_at, item.id);

  const updated = db.prepare('SELECT * FROM plans WHERE id = ?').get(item.id);
